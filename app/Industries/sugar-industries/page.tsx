...
const Page = () => {
  ...
  useEffect(() => {
    ...
    import("../../Utilities/data")
      .then((module) => {
        const { industries } = module;
        const retrievedData = industries[dataToGet as keyof typeof industries];
  ...
  return (
    <section className="relative min-h-screen flex flex-col items-center bg-gray-900 text-white overflow-hidden px-6 md:px-12">
      <div
        ...
        style={{
          backgroundImage: `url(${
            data.icon?.startsWith("http") ? data.icon : `/sia.png`
          })`,
        }}
      >
...
function ImageSliderWithContent({ data }: { data: IndustryData }) {
  console.log("This is the data that I am looking for!");
  console.log(data);
